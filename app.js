...
// ═══════════════════════════════════════════════════
// PEGAR ESTO EN app.js DESPUÉS DE LA FUNCIÓN barChart
// ═══════════════════════════════════════════════════

// ─── LINE CHART (SVG puro, sin Chart.js) ───
function lineChart(id, data, color, vFmt) {
  var el = $(id); if (!el || !data || !data.length) return;
  color = color || '#00d4ff';
  vFmt = vFmt || function(v){ return v; };
  var W = el.offsetWidth || 340, H = 160;
  ...
  var pts = data.map(function(d, i) {
    var x = padL + i * stepX;
    var y = padT + (1 - (d.v - minV) / range) * (H - padT - padB);
  ...
  var labels = '';
  var step = n <= 12 ? 1 : Math.ceil(n / 10);
  pts.forEach(function(p, i) {
    if (i % step === 0 || i === n - 1) {
      labels += '<text x="' + p.x.toFixed(1) + '" y="' + (H - 6) + '" text-anchor="middle" font-size="10" fill="var(--sub,#888)" font-family="system-ui">' + (data[i].l || '') + '</text>';
    }
  });

  var dots = pts.map(function(p, i) {
    return '<circle cx="' + p.x.toFixed(1) + '" cy="' + p.y.toFixed(1) + '" r="3.5" fill="' + color + '" stroke="var(--s1,#111)" stroke-width="1.5" style="cursor:pointer" onmouseenter="showLineTip(this,\'' + vFmt(p.v).replace(/'/g, "\\'") + '\',\'' + (data[i].l || '').replace(/'/g, "\\'") + '\')" onmouseleave="hideLineTip()"/>';
  }).join('');

  el.innerHTML = '<div id="line-tooltip" style="position:fixed;background:var(--s2,#1a1a2e);border:1px solid var(--b,#333);border-radius:6px;padding:6px 10px;font-size:11px;pointer-events:none;z-index:200;display:none;color:var(--t,#eee)"></div>'
    + '<svg width="100%" height="' + H + '" viewBox="0 0 ' + W + ' ' + H + '" preserveAspectRatio="none" style="display:block;overflow:visible">'
    + '<defs><linearGradient id="lg_' + id + '" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="' + color + '" stop-opacity=".25"/><stop offset="100%" stop-color="' + color + '" stop-opacity="0"/></linearGradient></defs>'
    + '<path d="' + areaD + '" fill="url(#lg_' + id + ')"/>'
    + '<path d="' + pathD + '" fill="none" stroke="' + color + '" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>'
    + dots + labels
    + '</svg>';
}

function showLineTip(el, val, label) {
  var tip = document.getElementById('line-tooltip'); if (!tip) return;
  tip.innerHTML = '<strong>' + label + '</strong><br>' + val;
...
function hideLineTip() {
  var tip = document.getElementById('line-tooltip'); if (tip) tip.style.display = 'none';
}

// ─── PIE CHART (SVG puro, sin Chart.js) ───
function pieChart(id, data, title) {
  var el = $(id); if (!el || !data || !data.length) return;
  var total = data.reduce(function(s, d) { return s + (d.v || 0); }, 0);
  if (total <= 0) { el.innerHTML = '<div style="color:var(--sub);font-size:12px;padding:20px">Sin datos</div>'; return; }

  var C = ['#00d4ff', '#ff3fa4', '#00e5a0', '#ffb020', '#a78bfa', '#33ddff', '#ff70c0', '#66efc0', '#ffd060'];
  ...
  data.forEach(function(d, i) {
    ...
    if (data.length === 1) {
      slices += '<circle cx="' + cx + '" cy="' + cy + '" r="' + R + '" fill="' + color + '" opacity=".85"/>';
    } else {
      slices += '<path d="M' + cx + ',' + cy + ' L' + x1.toFixed(2) + ',' + y1.toFixed(2) + ' A' + R + ',' + R + ' 0 ' + large + ',1 ' + x2.toFixed(2) + ',' + y2.toFixed(2) + ' Z" fill="' + color + '" opacity=".85" style="cursor:pointer" onmouseenter="this.style.opacity=1;this.style.transform=\'scale(1.03)\';this.style.transformOrigin=\'' + cx + 'px ' + cy + 'px\'" onmouseleave="this.style.opacity=.85;this.style.transform=\'\'"/>';
    }
    startAngle = endAngle;

    legend += '<div style="display:flex;align-items:center;gap:6px;font-size:11px;margin-bottom:3px">'
      + '<span style="width:8px;height:8px;border-radius:2px;background:' + color + ';flex-shrink:0"></span>'
      + '<span style="color:var(--sub,#888);flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + d.l + '</span>'
      + '<span style="font-family:var(--mono,monospace);color:var(--t,#eee);font-weight:600">' + (pct * 100).toFixed(1) + '%</span>'
      + '</div>';
  });

  // Donut hole
  slices += '<circle cx="' + cx + '" cy="' + cy + '" r="42" fill="var(--s1,#0d0d12)"/>';

  el.innerHTML = '<div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">'
    + '<svg width="180" height="180" viewBox="0 0 180 180" style="flex-shrink:0">' + slices + '</svg>'
    + '<div style="flex:1;min-width:120px">'
    + (title ? '<div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--sub,#888);margin-bottom:8px">' + title + '</div>' : '')
    + legend + '</div></div>';
}

// ─── FLUJO DE CAJA (faltante) ───
function renderFlujoCaja(resetFilter) {
  var bankData = [];
  ...
  if (sel && !sel.options.length) {
    var meses = {};
    bankData.forEach(function(t) { var m = t.date.slice(0, 7); meses[m] = 1; });
  ...
  var filtered = currentMonth === 'all' ? bankData : bankData.filter(function(t) { return t.date.startsWith(currentMonth); });
  
  var totalIn = filtered.reduce(function(s, t) { return s + (t.in || 0); }, 0);
  var totalOut = filtered.reduce(function(s, t) { return s + (t.out || 0); }, 0);
  
  var kpis = document.getElementById('kpi-flujo');
  if (kpis) {
    kpis.innerHTML = [
      { l: 'Ingresos', v: formatMoney(totalIn), f: filtered.length + ' movimientos' },
      { l: 'Egresos', v: formatMoney(totalOut), f: '', m: 1 },
      { l: 'Saldo neto', v: formatMoney(totalIn - totalOut), f: (totalIn - totalOut) >= 0 ? 'Positivo' : 'Negativo' }
    ].map(function(k) {
      return '<div class="kpi' + (k.m ? ' m' : '') + '"><div class="kpi-lbl">' + k.l + '</div><div class="kpi-val">' + k.v + '</div><div class="kpi-foot">' + k.f + '</div></div>';
    }).join('');
  }
  
  renderTablaFlujo(filtered.slice(0, 100), false, '');
}

function asociarProveedor(desc) {
  var cat = prompt('Categoría para "' + desc + '":\n(Gas, Agua, Arriendo, Insumos, Ingreso Web, etc.)');
  if (!cat || !cat.trim()) return;
  var reglas = {};
  try { reglas = JSON.parse(localStorage.getItem('reglas_prov') || '{}'); } catch(e) {}
  reglas[desc.toUpperCase()] = cat.trim();
  localStorage.setItem('reglas_prov', JSON.stringify(reglas));
  ...
  RECIPES.forEach(function(r) {
    ...
    r.ingredients.forEach(function(ing) {
      ...
      if (found) {
        ...
        else if (found.unit === 'mL' && ing.unit === 'L') qtyInBase = qty * 1000;
...
function setStockTab(tab) {
  document.querySelectorAll('.stock-tab').forEach(function(t) { t.classList.remove('on'); });
  var el = document.getElementById('st-' + tab);
  if (el) el.classList.add('on');
  document.querySelectorAll('.stock-view').forEach(function(v) { v.style.display = 'none'; });
  var view = document.getElementById('sv-' + tab);
